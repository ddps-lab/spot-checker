...
def spot_log_parse_log_data_to_csv(input_file, output_file):
    ...
    with open(output_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["DDDRequestTime", "InstanceType", "Region", "AZ", "Code", "RawCode", "LoopIndex", "ProvisioningState", "PowerState", "RequestCreateTime", "StatusUpdateTime", "Timestamp"])  # 헤더
        ...
        for line in lines:
            ...
            writer.writerow([
                ...
                log_json.get('LoopIndex', ''),
                log_json.get('ProvisioningState', ''),
...
def terminate_log_parse_log_data_to_csv(input_file, output_file):
    ...
    with open(output_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["LaunchTime", "TerminateTime", "BillingTime", "InstanceId", "InstanceState", "InstanceType", "AvailabilityZone", "LoopIndex"])  # 헤더

        for line in lines:
            log_timestamp, log_data = line.split(' ', 1)
            log_json = json.loads(log_data.strip())
            writer.writerow([log_json['LaunchTime'], log_json['TerminateTime'], log_json['BillingTime'], log_json['InstanceId'], log_json['InstanceState'], log_json['InstanceType'], log_json['AvailabilityZone'], log_json['LoopIndex']])
...
def pending_log_parse_log_data_to_csv(input_file, output_file):
    ...
    with open(output_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["Timestamp", "InstanceId", "InstanceState", "Region", "LoopIndex"])  # 헤더

        for line in lines:
            log_timestamp, log_data = line.split(' ', 1)
            log_json = json.loads(log_data.strip())
            writer.writerow([log_json['Timestamp'], log_json['InstanceId'], log_json['InstanceState'], log_json['Region'], log_json['LoopIndex']])
...
def main():
    ...
    # milliseconds_start_time = datetime_to_utc_milliseconds("2024-08-09 00:00")
    # milliseconds_end_time = datetime_to_utc_milliseconds("2024-08-10 00:00")
