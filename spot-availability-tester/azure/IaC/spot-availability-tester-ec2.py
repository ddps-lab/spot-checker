...
def test_azure_spot_vm(
    ...
    try:
        ...
        if is_odd_minute:
            ...
            nic_index = (base_nic_index % (AZURE_NIC_POOL_SIZE // 2)) + (
                AZURE_NIC_POOL_SIZE // 2
            )

        nic_name = f"{PREFIX}-{region_normalized}-nic-{nic_index}"

        print(
            f"[DEBUG NIC] minute={current_minute} ({'odd' if is_odd_minute else 'even'}), row_index={row_index}, pool_size={AZURE_NIC_POOL_SIZE}, base_index={base_nic_index}, nic_index={nic_index}, nic_name={nic_name}"
        )
        ...
        for loop_idx in range(max_loops):
            try:
                ...
                print(
                    f"[DEBUG] Loop {loop_idx + 1}/{max_loops} - PowerState: {power_state}, ProvisioningState: {provisioning_state}"
                )

                # ProvisioningState가 "OverconstrainedZonalAllocationRequest"이거나 PowerState가 "stopped"이면 break
                if (
                    provisioning_state == "OverconstrainedZonalAllocationRequest"
                    or power_state == "stopped"
                ):
                    print(
                        f"[DEBUG] 조건 충족으로 루프 종료 - ProvisioningState: {provisioning_state}, PowerState: {power_state}"
                    )
                ...
                if error_code not in ["ResourceNotFound", "NotFound"]:
                    print(
                        f"[WARN] VM 상태 조회 실패 (루프 {loop_idx + 1}): {error_code}"
                    )
                ...
                print(
                    f"[WARN] VM 상태 조회 중 예외 (루프 {loop_idx + 1}): {str(e)[:100]}"
                )
        ...
        try:
                ...
                if vm_resource.priority == "Spot":
                    spot_info = {
                        ...
                        "eviction_policy": getattr(
                            vm_resource, "eviction_policy", None
                        ),
                        "billing_profile": getattr(
                            vm_resource.billing_profile, "max_price", None
                        )
                        if vm_resource.billing_profile
                        else None,
                    }
            except Exception as e:
            ...
            print(
                f"[DEBUG] 최종 VM 상태 - PowerState: {power_state}, ProvisioningState: {provisioning_state}"
            )
        ...
        try:
            ...
            print(
                f"[DEBUG] Called terminate Lambda: {terminate_lambda_name} for VM: {vm_name} in {resource_group_name} (region: {region_display_name})"
            )
        except Exception as e:
