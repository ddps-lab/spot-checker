...
resource "aws_lambda_function" "lambda" {
  function_name = "${var.prefix}-terminate-orphan-disk"
  architectures = ["x86_64"]
  memory_size   = 256  # Azure SDK 사용으로 메모리 증가
  timeout       = 60   # Azure API 호출 시간 고려
  runtime       = "python3.11"
  handler       = "terminate-orphan-disks.lambda_handler"
  filename      = "terminate-orphan-disks.zip"
...
resource "aws_cloudwatch_event_rule" "eventbridge-rule" {
  name                = "${var.prefix}-terminate-orphan-disk"
...
resource "aws_cloudwatch_event_target" "eventbridge-target" {
  ...
  target_id = "${var.prefix}-terminate-orphan-disk"
