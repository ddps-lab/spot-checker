...
def create_log_stream(log_group_name, log_stream_name, logs_client):
    ...
    if (
        not response["logStreams"]
        or response["logStreams"][0]["logStreamName"] != log_stream_name
    ):
...
async def create_nic_pool_async(
    ...
    print(f"\n⚡ NIC 풀 비동기 생성 시작")
    print(
        f"   총 {len(azure_test_regions)} 리전 × {azure_nic_pool_size}개 = {total_nics}개 NIC"
    )
    ...
    async def create_single_nic(region_display_name, nic_index):
        ...
        azure_region = AZURE_REGION_MAP.get(
            region_display_name, region_display_name.lower().replace(" ", "")
        )
    ...
    for coro in asyncio.as_completed(tasks):
        ...
        print(
            f"\r   진행: {completed}/{len(tasks)} ({progress:.1f}%)", end="", flush=True
        )
...
def main():
    ...
    spawn_rate = (
        "rate(1 minute)"
        if variables.spawn_rate == 1
        else f"rate({variables.spawn_rate} minutes)"
    )
    ...
    if azure_test_regions:
        ...
        print(
            f"   총 생성될 NIC 개수: {len(azure_test_regions) * azure_nic_pool_size}개"
        )
    else:
        print(
    ...
    # Dispatcher 빌드 (서버리스 모드)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    tf_project_dir = os.path.join(script_dir, "IaC")
    dispatcher_dir = os.path.join(tf_project_dir, "dispatcher")

    if use_ec2 == "false":
        csv_src = os.path.join(script_dir, "azure.csv")
        csv_dst = os.path.join(dispatcher_dir, "data.csv")
        shutil.copy(csv_src, csv_dst)
        print(f"\n📋 Copied {csv_src} -> {csv_dst}")
        run_command(["bash", os.path.join(dispatcher_dir, "build.sh")])
        print(f"✅ Dispatcher build complete")
    ...
    for idx, region in enumerate(regions):
        ...
        if is_first_region:
            azure_test_regions_str = (
                '["' + '","'.join(azure_test_regions) + '"]'
                if azure_test_regions
                else "[]"
            )
        else:
        ...
        terraform_vars = [
            ...
            f"prefix={prefix}",
            "--var",
            f"awscli_profile={awscli_profile}",
            # CloudWatch Logs
            "--var",
            f"log_group_name={log_group_name}",
            "--var",
            f"spot_log_stream_name={spot_log_stream_name}",
            "--var",
            f"terminate_log_stream_name={terminate_log_stream_name}",
            "--var",
            f"pending_log_stream_name={pending_log_stream_name}",
            # Lambda 설정
            "--var",
            f"lambda_rate={spawn_rate}",
            "--var",
            f"use_ec2={use_ec2}",
            "--var",
            f"describe_rate={describe_rate}",
            # Azure 인증 정보
            "--var",
            ...
            "--var",
            f"azure_client_secret={azure_client_secret}",
            "--var",
            f"azure_resource_group={azure_resource_group}",
            # Azure 테스트 설정 (첫 번째 region에서만)
            "--var",
            f"azure_test_regions={azure_test_regions_str}",
            "--var",
            f"azure_nic_pool_size_runtime={azure_nic_pool_size}",  # Lambda 환경변수용 (실제 NIC 개수)
        ]
