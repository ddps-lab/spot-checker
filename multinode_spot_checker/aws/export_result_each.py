...
def change_log_parse_log_data_to_csv(input_file, output_file):
...
def merge_csv_files(log_type, regions, result_folder_path, maxinst):
    df_list = []
    
    for region in regions:
        file_path = f"./result_data/{result_folder_path}/{region}/{log_type}/max_{maxinst}.csv"
...
def main():
    ...
    while True:
        ...
        result_folder_path = f"{formatted_date}-max{maxinst}-{ranstr}"
    ...
    merge_csv_files("change", regions, result_folder_path, maxinst)
    merge_csv_files("init", regions, result_folder_path, maxinst)
