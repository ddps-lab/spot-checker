...
resource "aws_lambda_function" "lambda" {
  function_name = "${var.prefix}-get-spot-interruption"
  ...
  environment {
    variables = {
      ...
      LOG_STREAM_NAME   = var.log_stream_name_interruption,
    }
  }
}
...
resource "aws_cloudwatch_event_rule" "eventbridge-rule" {
  name                = "${var.prefix}-get-spot-interruption"
...
resource "aws_cloudwatch_event_target" "eventbridge-target" {
  ...
  target_id = "${var.prefix}-get-spot-interruption"
