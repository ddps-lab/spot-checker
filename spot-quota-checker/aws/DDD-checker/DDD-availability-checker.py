...
def query_availability(session, table_name):
    dynamodb = session.resource('dynamodb')
    table = dynamodb.Table(table_name)

    response = table.query(
        KeyConditionExpression=boto3.dynamodb.conditions.Key('key').eq('isAble')
    )
    return response['Items']

def update_dynamodb_table(session, table_name, updatedict):
    dynamodb = session.resource('dynamodb')
    table = dynamodb.Table(table_name)

    update_expression = 'SET ' + ', '.join([f"{k} = :{k}" for k in updatedict.keys()])
    expression_attribute_values = {f":{k}": v for k, v in updatedict.items()}
...
def main():
    boto3_session_list = []
    table_name = "ddd-available-checker"
    ...
    for region_index, region in enumerate(regions):
        session = boto3_session_list[region_index]
        ...
        update_dynamodb_table(session, table_name, newDict)
